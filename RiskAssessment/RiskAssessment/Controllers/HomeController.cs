            ...
            if (assessmentType == 1)
            {
                ...
                var _model =
                      ...
                      .Select(x => new ActionPlanModel()
                      {
                          AssessmentNo = x.AssessmentNo,
                          AssessmentTypeID = 1,
                          Dlist = _ctx.vw_HealthRisk.Where(y => y.AssessmentNo == assessment.AssessmentNo).Select(z => new HealthPlanModel
                          {
                              Risk = z.risk,
                              Nlist = _ctx.vw_ActionPlanDiet.Where(s => s.UID == z.UID).Select(t => new NutrientModel
                              {
                                  Nutrient = t.Nutrient,
                                  Description = t.SolutionDesc,
                                  ImgDir = t.imgDir
                              }).ToList(),
                              Elist = _ctx.vw_QuestionID_Activity.Where(k => k.UID == z.UID).Select(m => new ExerciseModel
                              {
